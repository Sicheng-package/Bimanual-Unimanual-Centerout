%THIS CODE IS CREATED BY SICHENG OVO
function bimanual_L()
...
startTask();
    ...
    function checkRemoteSignal()
        ...
        while true
            ...
            switch op
                case 2  % TARGET_DONE: [op][trial:uint16][two doubles:16B]
                    rTrial = double(typecast(uint8(payload(1:2)), 'uint16'));
                    if rTrial ~= trialIndex
                        continue;
                    end
                    times = typecast(uint8(payload(3:end)), 'double');  % 2 doubles
                ...
                case 4  % CENTER: [op][trial:uint16]
                    ...
                    if localCenterDone && (remoteCenterIdx >= trialIndex)&& ~targetActivated
                        activateNextTarget();
                    end

                case 5  % ACK4: [op][trial:uint16]
        ...
        function L = payloadLen(op_)
            switch op_
                case 2,  L = 18; % 2 bytes trial + 16 bytes (2 doubles)
                case 3,  L = 0;
                case 4,  L = 2;  % trial:uint16
                case 5,  L = 2;  % trial:uint16
                case 10, L = 0;
                case 11, L = 0;
                otherwise, L = 0;
            end
        ...
        if centerActivated
            if inCenter(x, y)
                ...
                elseif tNow >= centerEnterTime + centerHoldDuration
                    ...
                    if localCenterDone && ~targetActivated && (remoteCenterIdx >= trialIndex) 
                        activateNextTarget();
                    end
                end
            else
    ...
    function sendTTL(signalType)
        ...
        switch signalType
            ...
            case 3
                ...
                write(daqSession,[1 0 0 0 0]); write(daqSession,[1 1 0 0 0]);

            case 4

                write(daqSession,[0 1 1 0 0]); write(daqSession,[1 1 0 0 0]);
        end
    end
