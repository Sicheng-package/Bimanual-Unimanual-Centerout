...
%task parameters
        ...
        if targetActivated
            if inTarget(x, y, currentTarget)
                ...
                elseif ~targetReachedOnce && tNow >= hoverEnterTime + targetHoldDuration
                    ...
                    % >>> 结束条件改为按“成功次数”判定；maxCycles 仅作安全上限
                    if successCount >= goalSuccesses
                        disp('Goal successes achieved. Stopping...');
                        delete(timerObj);
                        close(fig);
                        return;
                    elseif cyclesCompleted >= maxCycles
                        disp('Max cycles reached (safety cap). Stopping...');
                        delete(timerObj);
                        close(fig);
                        return;
                    else
    ...
    function sendTTL(signalType)
        ...
        switch signalType
            ...
            % case 4
            % 
            %     write(daqSession,[0 0 0 1 0]); pause(0.01); write(daqSession,[0 0 0 0 0]);
            % case 5
            % 
            %     write(daqSession,[0 0 0 0 1]); pause(0.01); write(daqSession,[0 0 0 0 0]);
            %     %
                % case {3,4}
                %     ttlEnd = nowRel;
